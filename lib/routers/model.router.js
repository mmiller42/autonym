...
class ModelRouter {
	...
	find (req, res, next) {
		req.Model.find(req.query, req.filter, req.meta)
			.then(results => {
	...
	create (req, res, next) {
		req.Model.create(req.data, req.meta)
			.then(result => {
	...
	findOne (req, res, next) {
		req.Model.findOne(req.resourceId, req.filter, req.meta)
			.then(result => {
	...
	findOneAndUpdate (req, res, next) {
		req.Model.findOneAndUpdate(req.resourceId, req.data, req.filter, req.completeResource, req.meta)
			.then(result => {
				res.status(HTTP.OK).data = result;
				next();
			})
	...
	findOneAndDelete (req, res, next) {
		req.Model.findOneAndDelete(req.resourceId, req.filter, req.meta)
			.then(() => {
