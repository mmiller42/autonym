...
class ModelRouter {
	...
	find(req, res, next) {
		req.Model.find(req.query, req.filter).then(results => {
	...
	findOne(req, res, next) {
		req.Model.findOne(req.resourceId, req.filter).then(result => {
	...
	findOneAndUpdate(req, res, next) {
		req.Model.findOneAndUpdate(req.resourceId, req.data, req.filter, req.completeResource).then(result => {
			res.status(_httpStatusCodes2.default.OK).data = result;
			next();
		}).catch(err => next(err));
	}
	...
	findOneAndDelete(req, res, next) {
		req.Model.findOneAndDelete(req.resourceId, req.filter).then(() => {
