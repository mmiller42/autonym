...
class Model {
	...
	static findOneAndUpdate(resourceId, body, filter, completeResource) {
		return this._findOneAndUpdate ? this._findOneAndUpdate(resourceId, this.serialize(body), filter, this.serialize(completeResource)).then(resource => this.unserialize(resource)) : Promise.reject(new _autonymClientErrors.MethodNotAllowedError(`findOneAndUpdate is not implemented for the resource "${ this.name }".`));
	}
