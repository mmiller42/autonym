...
class Model {
	...
	static create (body, filter, meta) {
		return this._create
			? this._create(this.serialize(body), filter, meta).then(resource => this.unserialize(resource))
	...
	static find (query, filter, meta) {
		return this._find
			? this._find(query, filter, meta).then(resources => resources.map(resource => this.unserialize(resource)))
	...
	static findOne (resourceId, filter, meta) {
		return this._findOne
			? this._findOne(resourceId, filter, meta).then(resource => this.unserialize(resource))
	...
	static findOneAndUpdate (resourceId, body, filter, completeResource, meta) {
		return this._findOneAndUpdate
			? this._findOneAndUpdate(resourceId, this.serialize(body), filter, this.serialize(completeResource), meta)
	...
	static findOneAndDelete (resourceId, filter, meta) {
		return this._findOneAndDelete
			? this._findOneAndDelete(resourceId, filter, meta)
