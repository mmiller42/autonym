...
class Model {
	...
	static create(body, filter) {
		return this._create ? this._create(this.serialize(body), filter).then(resource => this.unserialize(resource)) : Promise.reject(new _autonymClientErrors.MethodNotAllowedError(`create is not implemented for the resource "${ this.name }".`));
	}

	static find(query, filter) {
		return this._find ? this._find(query, filter).then(resources => resources.map(resource => this.unserialize(resource))) : Promise.reject(new _autonymClientErrors.MethodNotAllowedError(`find is not implemented for the resource "${ this.name }".`));
	}

	static findOne(resourceId, filter) {
		return this._findOne ? this._findOne(resourceId, filter).then(resource => this.unserialize(resource)) : Promise.reject(new _autonymClientErrors.MethodNotAllowedError(`findOne is not implemented for the resource "${ this.name }".`));
	}

	static findOneAndUpdate(resourceId, body, filter) {
		return this._findOneAndUpdate ? this._findOneAndUpdate(resourceId, this.serialize(body), filter).then(resource => this.unserialize(resource)) : Promise.reject(new _autonymClientErrors.MethodNotAllowedError(`findOneAndUpdate is not implemented for the resource "${ this.name }".`));
	}

	static findOneAndDelete(resourceId, filter) {
		return this._findOneAndDelete ? this._findOneAndDelete(resourceId, filter) : Promise.reject(new _autonymClientErrors.MethodNotAllowedError(`findOneAndDelete is not implemented for the resource "${ this.name }".`));
	}
